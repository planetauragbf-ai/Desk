...
2. Dans **SQL Editor**, exécutez les fichiers de [`supabase/migrations/`](supabase/migrations) **dans l'ordre** (`0001_init.sql` → `0012_chat_calendrier.sql`) : tables, sécurité RLS, trigger de création de profil, chat temps réel, liens & outils, module stock, accès par application, dossiers, contrôle fin des comptes.
