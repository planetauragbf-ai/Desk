...
2. Dans **SQL Editor**, exécutez les fichiers de [`supabase/migrations/`](supabase/migrations) **dans l'ordre** (`0001_init.sql` → `0009_dossiers.sql`) : tables, sécurité RLS, trigger de création de profil, chat temps réel, liens & outils, module stock, accès par application, dossiers.
...
```
supabase/
  migrations/                # schéma + RLS (0001 → 0009 : init, accès, branding, tâches, validation, chat & liens, stock, desk, dossiers)
