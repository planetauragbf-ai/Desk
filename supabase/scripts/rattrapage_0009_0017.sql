...
create policy "links_all" on public.links for all to authenticated
  using (public.is_internal()) with check (public.is_internal());
-- La table des dossiers n'existe que si 0009_dossiers.sql a été exécuté :
-- on ne bloque pas la migration si ce n'est pas encore le cas.
do $$
begin
  if to_regclass('public.folders') is not null then
    execute 'drop policy if exists "folders_all" on public.folders';
    execute 'create policy "folders_all" on public.folders for all to authenticated using (public.is_internal()) with check (public.is_internal())';
  end if;
end $$;
