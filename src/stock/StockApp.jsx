...
// Tarifs fournitures par défaut (€ HT / unité), par format de colisage.
// Paramétrables par adhérent dans la grille (grille.fournitures).
const FOURN_DEFAUT={C1:1.20,C2:2.05,C3:2.39,C6:3.37,C12:6.19,M1:3.41,PAL:12,DPAL:6};
const fournPrix=(grille)=>({...FOURN_DEFAUT,...((grille&&grille.fournitures)||{})});
...
// Fournitures facturées au tarif de l'adhérent (grille.fournitures), par format.
const fPrix=fournPrix(adh?.grille);
const fourCost=f.colis.reduce((s,c)=>s+(fPrix[c.format]||0)*c.qty,0);
...
{facFour&&f.colis.filter(c=>c.qty>0).length>0&&<div style={{marginTop:8,background:P.ams,borderRadius:8,padding:10,border:`1px solid ${P.am}40`}}>
<div style={{fontSize:10,fontWeight:700,color:P.am,marginBottom:6}}>🧾 FOURNITURES FACTURÉES — transport et/ou assurance non pris en charge par Planet Aura</div>
{f.colis.filter(c=>c.qty>0).map((c,i)=>{const fmt=COLIS_FORMATS.find(x=>x.id===c.format);const pu=fPrix[c.format]||0;return <div key={i} style={{display:"flex",justifyContent:"space-between",gap:8,fontSize:11,padding:"3px 0",borderBottom:`1px solid ${P.am}20`}}><span>{c.qty} × {fmt?.label||c.format} <span style={{color:P.tm}}>({pu.toFixed(2)} € /u)</span></span><b>{(pu*c.qty).toFixed(2)} €</b></div>;})}
<div style={{display:"flex",justifyContent:"space-between",fontSize:12,fontWeight:700,color:P.am,paddingTop:6}}><span>Total fournitures</span><span>{fourCost.toFixed(2)} €</span></div>
<div style={{fontSize:9,color:P.tm,marginTop:4}}>Quantités reprises du colisage ci-dessus (modifiez-les là pour ajuster) — prix selon la grille de l'adhérent.</div>
</div>}
...
const updFourn=(id,v)=>{sD({...d,adherents:d.adherents.map(a=>a.id===sel?{...a,grille:{...a.grille,fournitures:{...fournPrix(a.grille),[id]:+v}}}:a)});};
...
return <div><h3 style={{color:P.tx,margin:"0 0 14px"}}>Grille Tarifaire</h3><Inp label="Adhérent" value={sel} options={d.adherents.filter(a=>a.stockageActif).map(a=>({value:a.id,label:a.name}))} onChange={sSel}/>{adh&&<div style={{display:"grid",gap:12,marginTop:14}}>{sections.map(sc=><Card key={sc.s}><div style={{fontSize:11,fontWeight:700,color:P.ac,marginBottom:10}}>{sc.s}</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:8}}>{sc.f.map(fi=><div key={fi.k} style={{display:"flex",flexDirection:"column",gap:3}}><label style={{fontSize:10,color:P.tm,fontWeight:600}}>{fi.l}</label><div style={{display:"flex",gap:4,alignItems:"center"}}><input type="number" key={sel+fi.k} defaultValue={adh.grille[fi.k]} onBlur={e=>{if(e.target.value!==""&&e.target.value!==String(adh.grille[fi.k]))upd(fi.k,e.target.value);}} style={{flex:1,fontFamily:FN,fontSize:13,background:P.bg,color:P.tx,border:`1px solid ${P.bd}`,borderRadius:6,padding:"7px 10px",outline:"none"}}/><span style={{fontSize:9,color:P.tm,whiteSpace:"nowrap",minWidth:60}}>{fi.u}</span></div></div>)}</div></Card>)}<Card><div style={{fontSize:11,fontWeight:700,color:P.ac,marginBottom:4}}>FOURNITURES</div><div style={{fontSize:10,color:P.tm,marginBottom:10}}>Prix € HT par unité (carton ou palette) — facturé sur les sorties quand le transport et/ou l'assurance ne sont pas pris en charge par Planet Aura.</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:8}}>{COLIS_FORMATS.map(cf=>{const px=fournPrix(adh.grille);const pal=cf.id==="PAL"||cf.id==="DPAL";return <div key={cf.id} style={{display:"flex",flexDirection:"column",gap:3}}><label style={{fontSize:10,color:P.tm,fontWeight:600}}>{pal?"🟫":"📦"} {cf.label}</label><div style={{display:"flex",gap:4,alignItems:"center"}}><input type="number" step="0.01" key={sel+"F"+cf.id} defaultValue={px[cf.id]} onBlur={e=>{if(e.target.value!==""&&+e.target.value!==+px[cf.id])updFourn(cf.id,e.target.value);}} style={{flex:1,fontFamily:FN,fontSize:13,background:P.bg,color:P.tx,border:`1px solid ${P.bd}`,borderRadius:6,padding:"7px 10px",outline:"none"}}/><span style={{fontSize:9,color:P.tm,whiteSpace:"nowrap",minWidth:40}}>€ HT</span></div></div>;})}</div></Card></div>}</div>;}
